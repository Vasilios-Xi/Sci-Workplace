...
import { Bot, Braces, ChevronRight, Database, Download, Eye, EyeOff, HardDrive, Info, KeyRound, Languages, Network, Palette, Power, ShieldCheck, SlidersHorizontal, Sparkles, Trash2, UserRound, X } from 'lucide-react';
import type { AgentCardExport, AgentDefinition, AgentDefinitionUpdate, AgentMemoryItem, AgentToolPolicy, BootstrapSnapshot, HarnessSettings, McpServerConfig, ModelProviderConfig, ModelProviderId, PermissionRule, ReasoningEffort, SecurityApprovalPolicy, SecurityPermissionCategory, UserProfileUpdate } from '@openlab/protocol';
...
type SettingsPage = 'providers' | 'agents' | 'user' | 'interface' | 'context' | 'skills' | 'mcp' | 'security' | 'data' | 'about';
const pages: Array<{
    id: SettingsPage;
    label: string;
    icon: typeof Bot;
    ...
    { id: 'user', label: zhCN.settingsPages.user, icon: UserRound },
...
interface SettingsProps {
    ...
    onUpdateUserProfile(update: UserProfileUpdate): Promise<void>;
...
export function SettingsModal({ open, snapshot, onClose, onInstallExtension, onApproveSkill, onConfigureMcp, onMcpAction, onUpdateSettings, onUpdateUserProfile, onCreateAgent, onUpdateAgent, onArchiveAgent, onImportAgent, onExportAgent, onSetAgentToolPolicy, onSetProjectAgentCapabilities, onListMemories, onCreateMemory, onUpdateMemory, onDeleteMemory, onClearMemories, onConfigureProvider, onRefreshProvider, onProviderOAuth, onProviderLogout, onExportDiagnostics, onBackupData }: SettingsProps) {
    ...
    return (<div className="modal-backdrop" onMouseDown={(event) => event.currentTarget === event.target && onClose()}>
      <div className="settings-modal" data-testid="settings-modal" role="dialog" aria-modal="true" aria-label={t("copy073")}>
        ...
        <main className="settings-content">
          {page === 'providers' && <ProviderSettings providers={snapshot.providers} models={snapshot.models} settings={snapshot.settings} onConfigure={onConfigureProvider} onRefresh={onRefreshProvider} onOAuthStart={onProviderOAuth} onOAuthLogout={onProviderLogout} onUpdateSettings={onUpdateSettings}/>}
          ...
          {page === 'user' && <UserSettings profile={snapshot.userProfile ?? { name: hanaZhCN.userSettings.defaultName, profile: '' }} onSave={onUpdateUserProfile}/>}
