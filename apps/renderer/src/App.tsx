...
export function App() {
  ...
  useEffect(() => {
    if (!openlab.connected || defaultModeApplied.current) return;
    defaultModeApplied.current = true;
    setMode(openlab.snapshot.primaryAgent.configured ? 'worktable' : 'chat');
  }, [openlab.connected, openlab.snapshot.primaryAgent.configured]);
