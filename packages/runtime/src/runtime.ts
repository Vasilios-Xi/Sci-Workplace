...
export class OpenLabRuntime {
    ...
    this.paperReaders = new PaperReaderService({
      ...
      createReportArtifact: (input, actor) => {
        const artifact = this.research.createObject({
          ...
          title: input.title,
          status: 'active',
          attributes: { kind: 'paper-deep-read-report', workbenchInstanceId: input.instanceId },
          attachments: [],
        ...
        const revision = this.artifactRevisions.create({
          ...
          provenance: {
            ...
            sessionId: this.#activeSessionId,
            agentId: this.agents?.primary()?.id ?? 'local-user',
            tool: 'sci.paper-reader:deep-read',
            plugin: { id: 'sci.paper-reader', version: '1.0.0' },
            inputObjectIds: [],
            inputFileHashes: {},
          },
        }, actor);
      ...
      model: () => this.isDemoMode() ? 'openlab-demo' : this.availableModel(this.#harnessSettings.utilityModel),
      translate: async (input, actor, signal) => {
        return await runPaperReaderTranslation(this.modelGenerations, input, actor, signal);
      },
      analyze: async (input, actor, signal) => {
        return await runPaperReaderAnalysis(this.modelGenerations, input, actor, signal);
      },
      onChanged: () => {
  ...
  decideGeneratedWorkbench(id: string, accepted: boolean) {
    const confirmed = this.generatedAppBlueprints.confirm(id, accepted, USER_ACTOR);
    const blueprint = accepted ? this.generatedAppBlueprints.build(id, USER_ACTOR) : confirmed;
    this.emitGeneratedBlueprints();
    this.emitArtifactRevisions();
    return blueprint;
  }
  ...
  createWorktable(input: { templateId?: string; title?: string; boundSessionId?: string; inputs?: Record<string, JsonValue> }): WorktableInstance {
    ...
    const instance = this.harnessWorkbenches.create({
      blueprintId: blueprint.id,
      ...(input.title ? { title: input.title } : {}),
      primaryConversationId: boundSessionId,
      ...(input.inputs ? { inputs: input.inputs } : {}),
    }, USER_ACTOR);
    if (blueprint.id === 'sci.paper-reader:deep-read') this.paperReaders.configure(instance, USER_ACTOR);
    this.emitWorktable();
    this.emitWorkbenchV1();
  ...
  private async executePaperReaderPanelTool(input: { worktableInstanceId?: string; tool: string; params: Record<string, JsonValue>; confirmed: boolean }): Promise<ToolExecutionResult> {
    ...
    const reusableAuthorization = reader.batchAuthorization?.status === 'active'
      ...
      && reader.batchAuthorization.documentSha256 === reader.mainDocument?.sha256;
    ...
    if (input.tool === 'paper.prepare') value = this.paperReaders.prepare(instanceId, USER_ACTOR);
      ...
      value = this.paperReaders.selectBlock(instanceId, input.params.blockId, USER_ACTOR);
      ...
      value = await this.paperReaders.translateBlocks(instanceId, input.params.blockIds, USER_ACTOR);
    } else if (input.tool === 'paper.ask') {
      if (typeof input.params.question !== 'string') throw new Error('来源问答参数无效');
      value = this.paperReaders.ask(instanceId, input.params.question, USER_ACTOR);
      ...
      value = this.paperReaders.regenerate(instanceId, input.params.blockIds, USER_ACTOR);
