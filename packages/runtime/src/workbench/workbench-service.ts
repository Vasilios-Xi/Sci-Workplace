...
import { validateWorktableLayout, type WorktableStore } from '../worktable/worktable-store.js';
...
export const CORE_WORKBENCH_BLUEPRINTS: WorkbenchBlueprintV1[] = [
  ...
  },
  {
    ...
    version: '2.0.0',
    title: '论文精读',
    description: '主论文与多个 SI 的全文双语、逐节/逐图/公式精读、证据链、复现与确定性质量门。',
    icon: 'book-open-text',
    ...
    panes: [
      {
        id: 'source', title: '原文',
        tabs: [{ id: 'paper-source', title: '深度精读 / 双语 / PDF', content: { kind: 'plugin-panel', pluginId: 'sci.paper-reader', panelId: 'source' }, pinned: true, openedAt: TEMPLATE_OPENED_AT }],
        activeTabId: 'paper-source',
      },
      {
        id: 'analysis', title: '精读',
        tabs: [{ id: 'paper-analysis', title: '章节、图表、复现与全局报告', content: { kind: 'plugin-panel', pluginId: 'sci.paper-reader', panelId: 'analysis' }, pinned: true, openedAt: TEMPLATE_OPENED_AT }],
    ...
    commands: ['paper.prepare', 'paper.deep-read-v2', 'paper.regenerate-selection', 'paper.regenerate-figure', 'paper.export-v2'],
  },
];
...
export class WorkbenchService {
  ...
  proposeLayout(input: {
    ...
    validateWorktableLayout(input.layout, input.panes);
