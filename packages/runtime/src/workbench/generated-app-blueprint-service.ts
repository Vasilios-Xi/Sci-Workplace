...
export class GeneratedAppBlueprintService {
  ...
  build(id: string, actor: EventActor): GeneratedAppBlueprintV1 {
    const current = this.get(id);
...
<title>${title}</title><style>:root{font-family:Inter,"Microsoft YaHei",sans-serif;color:#17211b;background:#f7faf8}body{margin:0;padding:28px}main{max-width:900px;margin:auto}section{background:white;border:1px solid #dce6df;border-radius:14px;padding:20px;box-shadow:0 10px 30px #163d2710}h1{font-size:22px;margin:0 0 8px}p{line-height:1.65;color:#526159}.badge{display:inline-block;padding:4px 8px;border-radius:999px;background:#e3f4e9;color:#176438;font-size:12px}textarea{box-sizing:border-box;width:100%;min-height:150px;border:1px solid #cbd8d0;border-radius:9px;padding:12px;resize:vertical}button{margin-top:10px;border:0;border-radius:8px;padding:9px 14px;color:white;background:#176438;cursor:pointer}</style></head>
<body><main><section><span class="badge">项目私有 · 离线沙箱</span><h1>${title}</h1><p>${prompt}</p><textarea id="notes" placeholder="在这里记录科研笔记……"></textarea><button id="save">保存到当前预览</button><p id="status" aria-live="polite"></p></section></main><script>document.getElementById('save').addEventListener('click',()=>{document.getElementById('status').textContent='已保存在当前沙箱会话中';});</script></body></html>`;
      const manifest = JSON.stringify({ schemaVersion: 1, blueprintId: current.id, entry: current.entry, networkDomains: [], hostCapabilities: [] }, null, 2);
