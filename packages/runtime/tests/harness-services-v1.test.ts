...
describe('prompt-generated application pipeline', () => {
  it('requires blueprint confirmation, builds a network-denied bundle, previews, accepts, mounts, and replays', async () => {
    const database = join(temporaryDirectory(), 'events.sqlite');
    const events = new SqliteEventStore(database);
    ...
    await expect(service.build(proposed.id, actor)).rejects.toThrow(/尚未通过/u);
    service.confirm(proposed.id, true, actor);
    const built = await service.build(proposed.id, actor);
  ...
  it('records model-generated code and rejects unsafe bundles before preview', async () => {
    const root = temporaryDirectory();
    const events = new SqliteEventStore(join(root, 'events.sqlite'));
    const artifacts: Array<Array<{ name: string; content: string }>> = [];
    const create = (generateCode: NonNullable<ConstructorParameters<typeof GeneratedAppBlueprintService>[0]['generateCode']>) => new GeneratedAppBlueprintService({
      projectId: 'project', events, generateCode,
      createArtifact: (input) => { artifacts.push(input.files); return { artifactId: `artifact-${input.blueprint.id}`, revisionId: `revision-${input.blueprint.id}` }; },
      publish: () => { throw new Error('not called'); }, mount: () => { throw new Error('not called'); },
    });
    const safe = create(async () => ({
      generationId: 'generation-safe',
      body: '<h1>证据核验矩阵</h1><button id="add">新增一行</button><div id="rows"></div>',
      style: '#rows{display:grid;gap:8px}',
      script: "document.getElementById('add').addEventListener('click',()=>document.getElementById('rows').append(document.createElement('p')));",
    }));
    const proposed = safe.propose('创建证据核验矩阵', actor);
    safe.confirm(proposed.id, true, actor);
    const built = await safe.build(proposed.id, actor);
    expect(built).toMatchObject({ status: 'preview', modelGenerationIds: ['generation-safe'] });
    expect(artifacts.at(-1)?.find((file) => file.name === 'index.html')?.content).toContain('证据核验矩阵');

    const unsafe = create(async () => ({ generationId: 'generation-unsafe', body: '<button onclick="fetch(\'https://evil.test\')">Run</button>', style: '', script: '' }));
    const unsafeProposal = unsafe.propose('联网抓取数据', actor);
    unsafe.confirm(unsafeProposal.id, true, actor);
    await expect(unsafe.build(unsafeProposal.id, actor)).resolves.toMatchObject({ status: 'failed', error: expect.stringMatching(/禁止|外部地址/u) });
    events.close();
  });
});
