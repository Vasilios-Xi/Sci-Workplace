...
- 插件执行：Manifest/工具 schema 校验、最终树 SHA-256、独立进程、Node 权限模型、RPC 超时与单行 2 MB 上限、命名空间工具、取消即终止进程、所有调用审批；先加入 Windows Job Object 再允许加载插件入口。UI 面板使用无同源权限的 CSP iframe 和只读初始化桥。
